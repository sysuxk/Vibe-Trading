...
export function CandlestickChart({ data, markers, indicators, height = 500 }: Props) {
  ...
  useEffect(() => {
    ...
    const marks = (markers || []).map(m => ({
      ...
      name: [`${m.side === "BUY" ? text.buy : text.sell} @ ${m.price}`, m.qty ? `${text.quantity}: ${m.qty}` : "", m.reason || ""].filter(Boolean).join("\n"),
    ...
    chart.setOption({
        ...
        formatter: (params: any) => {
          ...
          for (const p of params) {
            if (p.seriesName === "K" && Array.isArray(p.value)) {
              ...
              html += `<br/>${text.open}: ${open.toFixed(2)}&nbsp; ${text.high}: ${high.toFixed(2)}`;
              html += `<br/>${text.low}: ${low.toFixed(2)}&nbsp; ${text.close}: <span style="color:${clr}"><b>${close.toFixed(2)}</b> ${chg >= 0 ? "+" : ""}${chg.toFixed(2)} (${chg >= 0 ? "+" : ""}${pct}%)</span>`;
            } else if (p.seriesName === "Vol") {
              html += `<br/>${text.volume}: ${abbreviateNum(Number(p.value))}`;
      ...
      toolbox: {
        feature: { saveAsImage: { title: text.saveImage }, dataZoom: { title: { zoom: text.zoom, back: text.reset } }, restore: { title: text.reset } },
  ...
  }, [data, markers, baseData, indicatorCache, extraIndicators, sub, range, overlays, dark, text]);

  if (data.length === 0) {
    return <div className="text-muted-foreground text-sm p-4">{text.noPriceData}</div>;
  }
    ...
    <div>
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <div className="relative">
          ...
          >
            {text.overlayIndicators} ({overlays.size}) <ChevronDown className="h-3 w-3" />
          </button>
          {showMenu && (
            <div className="absolute top-full left-0 mt-1 z-50 bg-card border rounded-lg shadow-lg p-2 min-w-[160px]" onMouseLeave={() => setShowMenu(false)}>
              {["MA", "Channel"].map(group => (
                <div key={group}>
                  <p className="text-[9px] text-muted-foreground/50 uppercase tracking-wider px-1 pt-1">{group === "MA" ? text.overlayMA : text.overlayChannel}</p>
