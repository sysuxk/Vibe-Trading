...
export function Agent() {
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(t.uploadedPrefix.replace("{name}", result.filename));
    } catch (err) {
      toast.error(t.uploadFailedPrefix.replace("{message}", err instanceof Error ? err.message : t.unknownError));
    } finally {
      setUploading(false);
    }
  }, []);
